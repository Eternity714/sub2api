...
16:11-16:12 受控三模式对照：同一国际站、同一测试 Key、模型与提示词，每种 `replyType` 仅发一次生成 POST，均 HTTP 200。`stream` 生成终态 `succeeded`，其 ID 的 `/result` 三次只读 GET 均 404；`async` 生成返回 `running`，同 ID 的首次 GET HTTP 200、`running`；`json` 生成终态 `succeeded`，其 ID 的 `/result` 三次只读 GET 均 404。404 的重查间隔约五秒，所有查询都处于结果保留期内。三个模式都返回可解析的 ID，但实测只有 Async ID 可经 `/result` 查询；这是本次模型/接口的行为证据，不宣称其他模型或未来协议永远相同。未输出凭据、完整 ID、图片 URL 或响应体；未重试任何生成 POST。现有统一上游 Stream 的恢复契约仍未满足，维持 **no-go**。
...
- `python test/grsai_live_contract.py --self-test`：当前 16/16 通过，未访问真实上游（包含三模式对照、GET 404 阶段及不确定 POST 停止回归）；此前已记录的 Go 与前端全量结果未因本轮文档/探针改动重跑。
...
- `python test/grsai_live_contract.py --compare-modes`：明确人工触发，三模式各一次 POST、至多三次只读结果 GET；本轮实测 Stream/JSON 为 404、Async 为 200。此结果不能由普通 Stream live probe 绿灯代替。
...
1. 三模式受控测试已确认本次 Stream/JSON ID 查询为 404，而 Async ID 查询为 200。需向供应商确认 Stream/JSON ID 的 `/result` 支持范围和一致性，不以更多重复 POST 或放宽探针掩盖差异。评审产品契约与技术路线：例如统一使用可查询的原生 Async 作为上游任务并由本地提供 JSON 与轮询式 Stream，或显式收缩断线恢复承诺并重新审阅 PRD/AC-007；未经确认不得擅自削弱可恢复性。任何方案都须补对应真协议和断线/重启/账务回归。
