...
本地验证通过不等于上线通过。用户已提供国际站测试凭据（不入库）。一次真实上游契约探测已执行，但客户端返回 `transport_error`；用户报告在 2026-09-24 12:29（北京时间）看到一条成功调用记录。时间接近，但尚无任务 ID 或响应证据将该记录与探针逐项关联，不能据此判定 SSE 与结果查询契约通过；不得自动重复 POST，也不得在 live 门未通过时切入生产流量。生产灰度槽位和镜像须在部署前重新以 `gray-status.sh` 核对。

2026-09-24 只读排查：探针进程继承本机 `HTTPS_PROXY=http://127.0.0.1:7890`；Python 对不存在任务的 GET 通过该代理超时，绕过代理的相同 GET 迅速返回 404，curl 两种路径均可达。代理超时是首轮失败的可能原因，不证明生成 POST 未到达上游。应先取得 12:29 成功记录的任务 ID，并核对模型、请求时间和状态；对该 ID 只读查询结果。若无法取得 ID，不能通过重复 POST 找回首轮 SSE 证据。探针已改为允许国际站 `https://grsaiapi.com`，并对 HTTP 状态码和传输超时做脱敏分类；首次运行的旧版本没有分类信息。不得记录密钥、完整任务 ID、原始响应或图片 URL。
...
1. 对用户报告的 12:29 成功记录核对模型、任务 ID 和结果状态，并对该 ID 只读查询；首轮 SSE 未被探针确认，不能以成功调用记录替代。待确认本机传输路径可靠后，再单独决定是否需要新的一次完整 live probe；不得自动重复 POST。还须核查 Stream 断线恢复。
