...
1. 核查上游 15:23 请求记录，判明新 probe 的 404 来自生成 POST 还是结果 GET；在核查前禁止再发 POST。12:29 成功记录已证实生成成功，但因超过 2 小时保留期，其结果 GET 返回过期信息，不能用于当前结果查询契约验收。还须确认一次新任务的 SSE 与即时结果查询均通过，并核查 Stream 断线恢复。
