...
生产只读核查：2026-09-24 稳定槽 blue、候选 green 0%，两槽健康；两容器的 `stop_timeout` 均为 10 秒，灰度 Compose 未配置停止宽限期。应用的上游请求超时是 30 分钟，Worker 停机等待已开始请求完成。上线 Async 前，须通过受控运维流程把灰度槽位的停止宽限期设置为大于上游超时、HTTP 关闭和清理余量的总和，并在候选重建后验证生效。不能直接依赖当前 10 秒配置或手工绕开灰度脚本。
...
3. 先完成灰度槽位停止宽限期配置及复核，再在 0% 候选槽位用不可变 `sha-<commit>` 部署，验证健康检查、Nginx、人工关键路径和账务，分阶段开启 JSON、Async、Stream，逐步切流并观察，最终晋升。异常时只用灰度脚本切回稳定槽位，保留现场。
