...
“恢复运行时接手”不是继续维持已经断开的 HTTP 或 SSE 连接，而是后台继续处理已经被安全识别的任务。
流程如下：

1. GRS.AI 的上游 Stream 首次带回任务 ID 后，服务端立即将该 ID 连同用户、API Key、选中的账号、
   价格快照和冻结额度写入任务记录；这称为“任务 ID 已绑定”。
2. 若客户端随后断开，服务端停止向该客户端写 JSON 或 SSE 数据；断线本身不是取消，也不会创建新的
   上游生成请求。
3. 后台恢复运行时使用已保存的上游任务 ID 调用 GRS.AI 的
   `GET /v1/api/result?id=<上游任务ID>`，持续获知任务是仍在运行、成功、失败还是违规。
4. 成功任务按既定幂等规则结算一次；失败或违规任务释放冻结额度；仍在运行的任务按恢复策略稍后继续
   查询。
5. 客户端可用自己的 API Key 调用 Sub2API 的
   `GET /v1/api/result?id=<本地公开任务ID或已绑定上游任务ID>` 重新读取安全任务视图。
   该接口必须同时核验用户和 API Key 归属，因而不能读取其他用户或其他 Key 的任务。

若客户端在服务端获得并持久化上游任务 ID 之前断线，服务端无法安全地知道上游是否已经接收生成请求。
该记录转为 `manual_review`，不调用第二次生成、不猜测任务 ID，也不对用户自动收费。
...
异步任务按 `user_id`（跨 API Key）设置等待中 20、进行中 3 的持久容量门槛。受理事务对该用户串行化，先计数后插入任务；等待满额返回 429，不冻结余额。Worker 领取事务也对该用户串行化，先检查进行中名额再将等待任务转为进行中；满额跳过而不是占用任务租约。两条路径使用同一用户级事务锁，保证多实例下原子计数。仅 Async 参与；JSON/Stream 沿用现有请求并发限制。

“进行中”从首次成功领取开始，包含上游提交、已绑定结果轮询和不确定提交的安全核查期；该标记在租约到期和重新领取时保持不变。已经进入终态或仅剩结算重试时不再占用进行中名额。终态转换自然释放名额，拒绝受理/领取不创建新的冻结或上游 POST。需要真库并发测试证明 20/3 边界和重启恢复。

受理时设置的载荷 TTL 只清理不再等待首次提交的密文；尚未提交上游的排队任务及刚领取任务即使等待超过 TTL，仍可解密并执行。否则 3 个进行中任务持续超过 15 分钟时，已受理的队列任务会无故失效。绑定任务 ID 后立即删除载荷；过期且不再处于首次提交阶段的载荷可由清理器删除。
...
`--live`，并且每次运行最多创建一个 `nano-banana-2-lite` 任务。

探测只向上游发送 `replyType=stream`，逐帧验证 Content-Type、事件格式、稳定任务 ID、单调进度、终态和
`GET /v1/api/result` 一致性。上游 credits 与费用不纳入本次协议探测和发布门；本地冻结与结算
正确性仍须通过账务测试。仅离线自测进入 CI，真实探测不得进入 CI，也不得保存 Key、完整 prompt、图片
URL、完整原始响应。
